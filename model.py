...
def split_data(df: pd.DataFrame):
    y = df['Transported']
    X = df[["HomePlanet", "CryoSleep", "Age", "VIP", "Destination"]]

    return X, y


def open_data(path="data/spaceship_titanic_train.csv"):
    df = pd.read_csv(path)
    df = df[['Transported', "HomePlanet", "CryoSleep", "Age", "VIP", "Destination"]]
...
def preprocess_data(df: pd.DataFrame, test=True):
    ...
    to_encode = ['HomePlanet', 'Destination']
...
def load_model_and_predict(df, path="data/model_weights.mw"):
    ...
    encode_prediction_proba = {
        0: "Вы остались в том же измерении с вероятностью",
        1: "Вас переместило в другое измерение с вероятностью"
    }

    encode_prediction = {
        0: "Вы остались в том же измерении",
        1: "Вас переместило в другое измерение"
    }
